...
class SPHParticlesGPU:
    ...
    def __init__(self, n_particles):
        ...
        self.density = cp.zeros(n_particles, dtype=cp.float32)
        self.drho_dt = cp.zeros(n_particles, dtype=cp.float32)
...
class SPHSimulatorGPU:
    ...
    def _compute_drho_dt_vectorized(self):
        """Compute drho/dt using vectorized operations (no race condition)."""
        p = self.particles
        n = p.n
        ...
        # Contribution: m_j * (v_i - v_j) . gradW
        # For each i, sum over j
        contribution = p.mass[None, :] * (dvx * gradWx + dvy * gradWy)
        contribution = cp.where(mask, contribution, 0.0)

        # Sum contributions for each particle i (no race: each i gets its own sum)
        p.drho_dt = cp.sum(contribution, axis=1)

    def _compute_pressure(self):
        """Compute pressure using Tait EOS."""
        p = self.particles
        B = self.rho0 * self.c0**2 / self.gamma
        p.pressure = B * ((p.density / self.rho0)**self.gamma - 1.0)
        p.pressure = cp.maximum(p.pressure, 0.0)
    ...
    def _compute_forces_vectorized(self):
        ...
        # --- Pressure force ---
        # -m_j * (P_i/rho_i^2 + P_j/rho_j^2) * gradW
        rho_i = p.density[:, None]  # N x 1
        rho_j = p.density[None, :]  # 1 x N
        P_i = p.pressure[:, None]
        P_j = p.pressure[None, :]
        m_j = p.mass[None, :]

        pressure_term = P_i / (rho_i**2) + P_j / (rho_j**2)
        pressure_force_x = -m_j * pressure_term * gradWx
        pressure_force_y = -m_j * pressure_term * gradWy
    ...
    def step(self):
        ...
        # Compute drho/dt
        self._compute_drho_dt_vectorized()

        # Update density (half step)
        p.density += 0.5 * self.dt * p.drho_dt
        p.density = cp.maximum(p.density, 0.5 * self.rho0)
